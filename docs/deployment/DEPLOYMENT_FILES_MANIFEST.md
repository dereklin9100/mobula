...
| 应用 | Dockerfile | Dockerfile.prod | 说明 |
|------|-----------|-----------------|------|
| Web 应用 | ~800MB | ~800MB | 包含完整依赖确保 Next.js 正常运行 |
| Login 应用 | ~800MB | ~800MB | 同上 |
| Dashboard 应用 | ~800MB | ~800MB | 同上 |

> *实际大小取决于依赖和源代码体积。相对于早期版本，镜像大小有所增加，但确保了 Node.js 版本兼容性和依赖完整性*
...
- **基础镜像**: node:22
- **pnpm 版本**: 9.0.0
- **Node 版本**: >=20.9.0 (Next.js 16 要求)
- **构建目标**: builder, runner-web, runner-login, runner-dashboard
- **包含 turbo.json**: 是 ✅
- **副本依赖**: 包含完整 node_modules (生产部署需要)

### Dockerfile.prod 关键信息
- **基础镜像**: node:22
- **优化**: 完整依赖 + 健康检查
- **镜像大小**: 约 800MB-1GB (完整依赖)
- **启动命令**: pnpm dlx next start -p <port>

### docker-compose 关键信息
- **服务数**: 3 (web:3000, login:3001, dashboard:3002)
- **网络**: 自动创建 mobula 网络
- **重启策略 (prod)**: always
- **健康检查**: curl -f http://localhost:<port>
